...
export default function Home() {
  ...
  const loadViewer = useCallback(async () => {
    try {
      ...
      const payload = (await response.json()) as { user?: Viewer };
      setViewer(payload.user ?? null);
    } catch {
      // Keep board usable even if viewer profile fails to load.
    }
  }, []);
  ...
  const deleteLabelEverywhere = useCallback(
    async (label: string) => {
      ...
      try {
        ...
        setConfirmLabel((current) => (current === label ? null : current));
      }
    },
    ...
    <div className="relative min-h-screen bg-background text-foreground">
          ...
          <main className="relative w-full pb-0 pt-0">
            ...
            className={`relative sticky top-0 hidden h-screen shrink-0 flex-col overflow-hidden border-r transition-[width,opacity,transform,padding,border-color,background-color] duration-[420ms] ease-[cubic-bezier(0.22,1,0.36,1)] md:flex ${
              ...
              <div className="flex items-center justify-between px-4 py-2">
                <div className="flex items-center gap-2">
                  <p className="text-[11px] uppercase tracking-[0.24em] text-white/55">THE BOARD</p>
                  <span className="border border-white/12 bg-white/5 px-2 py-[2px] text-[9px] font-semibold uppercase tracking-[0.14em] text-white/55">
                    v3
                  </span>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto px-4 py-3">
                <div className="mb-4 flex items-center gap-2 rounded-lg border border-white/12 bg-white/5 px-2.5 py-2">
                  ...
                  <button
                    ...
                    className="border border-white/18 bg-white/6 px-2 py-1 text-[10px] uppercase tracking-[0.12em] text-white/70 transition hover:border-white/30 hover:text-white"
                  >
                    Sign out
                  </button>
                </div>
                  ...
                  <section>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {visibleSidebarLabelEntries.map((entry) => {
                        ...
                        const isDeleting = deletingLabel === label;
                        return (
                          <div
                            key={label}
                            className={`relative inline-flex items-center rounded-none border py-1 pl-3 pr-2 transition ${
                              isActive
                          ...
                          >
                            <button
                              type="button"
                              onClick={() => setActiveFilterLabel((current) => (current === label ? null : label))}
                            ...
                            {!isSystemLabel ? (
                              <button
                                ...
                                disabled={isDeleting}
                                className="ml-1 inline-flex items-center justify-center text-white/70 transition hover:text-white disabled:cursor-not-allowed disabled:opacity-40"
                              >
                                {isDeleting ? (
                                  <span className="text-[10px] leading-none">...</span>
                                ) : (
                                  <svg viewBox="0 0 24 24" aria-hidden="true" className="h-3.5 w-3.5">
                                    <path fill="currentColor" d="M18.3 5.71a1 1 0 0 0-1.41 0L12 10.59 7.11 5.7A1 1 0 0 0 5.7 7.12L10.58 12 5.7 16.89a1 1 0 1 0 1.41 1.41L12 13.41l4.89 4.89a1 1 0 0 0 1.41-1.41L13.41 12l4.89-4.88a1 1 0 0 0 0-1.41Z" />
                                  </svg>
                                )}
                              </button>
                            ) : null}
                            {!isSystemLabel && confirmLabel === label ? (
                              <div className="absolute left-0 top-[calc(100%+4px)] z-40 w-[160px] rounded-lg border border-white/16 bg-black/85 p-1.5 text-[11px] text-white/80 shadow-[0_10px_24px_rgba(0,0,0,0.45)]">
                                <p>Delete label?</p>
                                <div className="mt-1 flex gap-1">
                                  <button
                                    ...
                                    disabled={isDeleting}
                                    className="rounded-md border border-white/22 bg-white/8 px-1.5 py-0.5 text-[10px] text-white/90 transition hover:bg-white/12 disabled:cursor-not-allowed disabled:opacity-50"
                                  >
                                    {isDeleting ? "..." : "Yes"}
                                  </button>
                                  <button
                                    ...
                                    className="rounded-md border border-white/18 px-1.5 py-0.5 text-[10px] text-white/70 transition hover:border-white/32 hover:text-white/90"
                                  >
                                    No
                                  </button>
                                </div>
              ...
              {labelFilterEntries.length ? (
                <div className="mt-2.5 flex flex-wrap gap-1.5">
                  {labelFilterEntries.map((entry) => {
                    ...
                    const isDeleting = deletingLabel === label;
                    return (
                      <div key={`mobile-label-${label}`} className="inline-flex items-center gap-1">
                        <button
                          type="button"
                          onClick={() => setActiveFilterLabel((current) => (current === label ? null : label))}
                          className={`rounded-none border px-3 py-1 text-xs transition ${
            ...
            <section className="relative mb-6 hidden min-h-11 items-center px-2 py-2 text-sm text-muted-foreground md:flex">
              <div className="flex items-center gap-4 self-center">
                ...
                <p>
                  Showing <span className="font-semibold text-foreground">{filteredItems.length}</span> of {items.length} images
                </p>
              </div>
      ...
      {activeItem ? (
        ...
        >
          ...
          <p className="pointer-events-none absolute bottom-3 right-4 z-50 whitespace-nowrap text-sm text-white/85 tabular-nums md:bottom-5 md:right-6">
